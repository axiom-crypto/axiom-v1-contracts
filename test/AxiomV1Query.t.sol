...
contract AxiomV1QueryTest is Test {
    ...
    function testSendOffchainQuery() public {
        ...
        emit QueryInitiatedOffchain(
            keccakQueryResponse, 10 * 1000 * 1000 gwei, 16508329 + queryDeadlineInterval, address(12), ipfsHash
        );
        axiomQuery.sendOffchainQuery{value: 10 * 1000 * 1000 gwei}(keccakQueryResponse, payable(address(12)), ipfsHash);
        (uint256 payment, IAxiomV1Query.AxiomQueryState state, uint32 deadlineBlockNumber, address payable refundee) =
            axiomQuery.queries(keccakQueryResponse);
    ...
    function testSendOffchainQuery2() public {
        ...
        vm.expectEmit(false, false, false, true);
        emit QueryInitiatedOffchain(
            keccakQueryResponse, 2 ether, 16508329 + queryDeadlineInterval, address(12), ipfsHash
        );
        axiomQuery.sendOffchainQuery{value: 2 ether}(keccakQueryResponse, payable(address(12)), ipfsHash);
        (uint256 payment, IAxiomV1Query.AxiomQueryState state, uint32 deadlineBlockNumber, address payable refundee) =
            axiomQuery.queries(keccakQueryResponse);
    ...
    function testSendOnchainQuery() public {
        ...
        emit QueryInitiatedOnchain(
            keccakQueryResponse, 10 * 1000 * 1000 gwei, 16508329 + queryDeadlineInterval, address(12), queryHash
        );
        axiomQuery.sendQuery{value: 10 * 1000 * 1000 gwei}(keccakQueryResponse, payable(address(12)), query);
        (uint256 payment, IAxiomV1Query.AxiomQueryState state, uint32 deadlineBlockNumber, address payable refundee) =
            axiomQuery.queries(keccakQueryResponse);
        assert(payment == 10 * 1000 * 1000 gwei);
        assert(state == IAxiomV1Query.AxiomQueryState.Active);
    ...
    function testSendOnchainQuery_price_fail() public {
        ...
        vm.prank(address(100));
        vm.expectRevert();
        axiomQuery.sendQuery{value: 8 * 1000 * 1000 gwei}(keccakQueryResponse, payable(address(12)), query);
    }
    ...
    function testSendOnchainQuery_price2_fail() public {
        ...
        vm.prank(address(100));
        vm.expectRevert();
        axiomQuery.sendQuery(keccakQueryResponse, payable(address(12)), query);
    }
    ...
    function testSendOnchainQuery_price3_fail() public {
        ...
        vm.prank(address(100));
        vm.expectRevert();
        axiomQuery.sendQuery{value: 2_000_000_001 gwei}(keccakQueryResponse, payable(address(12)), query);
    }
    ...
    function testCollectRefund() public {
        ...
        vm.expectEmit(false, false, false, true);
        emit QueryRefunded(keccakQueryResponse, 10 * 1000 * 1000 gwei, 16508329 + queryDeadlineInterval, address(12));
    ...
    function testIsKeccakResultValid() public {
        testFulfillQueryVsMMR();
        assert(
            axiomQuery.isKeccakResultValid(
                bytes32(0x434bf7672b7657411e1824edbbea4cb990a1a6ee56d9be0e659774cbe8f956dd),
                bytes32(0xac95e2258648f8f86b3624bbc04b521fdf4b6ff03783b272e9861bff623530c7),
                bytes32(0x7e3cd333fb85a005b3eabed6bfcb2760966650d0a5377a80e3d2ed449f4bb63f)
            )
    ...
    function testIsPoseidonResultValid() public {
        testFulfillQueryVsMMR();
        assert(
            axiomQuery.isPoseidonResultValid(
    ...
    function setupVerifyOld() public returns (IAxiomV1Query.RecentMMRWitness memory mmrWitness) {
        deployRecent();
    ...
    function testVerifyResultVsMMR_afterMerkle_fail_recentMMR() public {
        ...
        axiom.setHistoricalRoot(
            16_525_312, keccak256(abi.encodePacked(mmrWitness.prevHash, mmrWitness.root, mmrWitness.numFinal))
        );
        vm.prank(address(111));
        vm.expectRevert();
        axiomQuery.verifyResultVsMMR(2, mmrWitness, proof);

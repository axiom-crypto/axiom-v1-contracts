...
contract AxiomV1QueryMock is IAxiomV1Query, AxiomV1Access, UUPSUpgradeable {
    ...
    function verifyResultVsMMR(uint32 mmrIdx, RecentMMRWitness calldata mmrWitness, bytes calldata proof)
        external
        onlyProver
    {
        requireNotFrozen();
    ...
    function sendQuery(bytes32 keccakQueryResponse, address payable refundee, bytes calldata query) external payable {
        ...
        emit QueryInitiatedOnchain(
            keccakQueryResponse, msg.value, uint32(block.number) + queryDeadlineInterval, refundee, queryHash
        );
    }
        ...
        payable
    {
        ...
        emit QueryInitiatedOffchain(
            keccakQueryResponse, msg.value, uint32(block.number) + queryDeadlineInterval, refundee, ipfsHash
        );
    }
    ...
    function collectRefund(bytes32 keccakQueryResponse) external {
        ...
        emit QueryRefunded(
            keccakQueryResponse, queryMetadata.payment, queryMetadata.deadlineBlockNumber, queryMetadata.refundee
        );
    }
    ...
    function isKeccakResultValid(
        ...
        return verifiedKeccakResults[keccak256(
            abi.encodePacked(keccakBlockResponse, keccakAccountResponse, keccakStorageResponse)
        )];
    }
    ...
    function isPoseidonResultValid(
        ...
        return verifiedPoseidonResults[keccak256(
            abi.encodePacked(poseidonBlockResponse, poseidonAccountResponse, poseidonStorageResponse)
        )];
    }
    ...
    function areResponsesValid(
        ...
        if (
            !verifiedKeccakResults[keccak256(
                abi.encodePacked(keccakBlockResponse, keccakAccountResponse, keccakStorageResponse)
            )]
        ) {
            return false;
        ...
        for (uint32 idx = 0; idx < blockResponses.length; idx++) {
            bytes32 leaf = keccak256(abi.encodePacked(blockResponses[idx].blockHash, blockResponses[idx].blockNumber));
        ...
        for (uint32 idx = 0; idx < accountResponses.length; idx++) {
            ...
            if (
                !isMerklePathValid(
                    keccakAccountResponse, leaf, accountResponses[idx].proof, accountResponses[idx].leafIdx
                )
            ) {
                return false;
            }
        ...
        for (uint32 idx = 0; idx < storageResponses.length; idx++) {
            ...
            if (
                !isMerklePathValid(
                    keccakStorageResponse, leaf, storageResponses[idx].proof, storageResponses[idx].leafIdx
                )
            ) {
                return false;
            }
    ...
    function _verifyResultVsMMR(uint32 mmrIdx, RecentMMRWitness calldata mmrWitness, bytes calldata proof)
        internal
        returns (bytes32)
    {
        requireNotFrozen();
        ...
        if (mmrLen > 0 && mmrLen <= mmrWitness.numFinal) {
            ...
            // Some of the claimed block hashes in `mmrWitness` were not committed to in `mmrWitness.root`
        ...
        // update the cache
        bytes32 keccakQueryResponse = keccak256(
            abi.encodePacked(
                response.keccakBlockResponse, response.keccakAccountResponse, response.keccakStorageResponse
            )
        );

        verifiedKeccakResults[keccakQueryResponse] = true;
        verifiedPoseidonResults[keccak256(
            abi.encodePacked(
                response.poseidonBlockResponse, response.poseidonAccountResponse, response.poseidonStorageResponse
            )
        )] = true;
    ...
    function getMMRQueryData(bytes calldata proof) internal pure returns (AxiomMMRQueryResponse memory) {
        return AxiomMMRQueryResponse({
            poseidonBlockResponse: bytes32(proof[384:384 + 32]),
            keccakBlockResponse: bytes32(
            ...
            keccakStorageResponse: bytes32(
                uint256(bytes32(proof[384 + 7 * 32:384 + 8 * 32])) << 128
                    | uint256(bytes32(proof[384 + 8 * 32:384 + 9 * 32]))
                ),
            historicalMMRKeccak: bytes32(
                uint256(bytes32(proof[384 + 9 * 32:384 + 10 * 32])) << 128
                    | uint256(bytes32(proof[384 + 10 * 32:384 + 11 * 32]))
                ),
            recentMMRKeccak: bytes32(
                uint256(bytes32(proof[384 + 11 * 32:384 + 12 * 32])) << 128
                    | uint256(bytes32(proof[384 + 12 * 32:384 + 13 * 32]))
                )
        });
    }
